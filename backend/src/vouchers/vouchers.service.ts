...
export class VouchersService {
  ...
  async findAll(): Promise<Voucher[]> {
    return this.voucherRepo.find({ relations: ['package', 'redeemedByUser'], order: { createdAt: 'DESC' } });
  }

  async redeem(code: string, user: User, routerId: string, subscriptionsService: any): Promise<any> {
    const voucher = await this.voucherRepo.findOne({ where: { code }, relations: ['package'] });
    if (!voucher) throw new NotFoundException('Invalid voucher code');
    if (voucher.isRedeemed) throw new BadRequestException('Voucher has already been redeemed');

    // Purchase using SubscriptionsService
    const sub = await subscriptionsService.purchase(user.id, voucher.package.id, routerId);
    
    // Activate immediately
    const activated = await subscriptionsService.activate(sub.id, 'voucher', voucher.code);
